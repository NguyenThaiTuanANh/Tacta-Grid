...
public class ItemSpawner : MonoBehaviour
{
    ...
    [SerializeField] private float hintDelayAfterSpawn = 5.5f; // Delay trước khi hiển thị hint
    [SerializeField] private float delayBetweenHints = 5.5f; // Delay giữa 2 hints khi cả 2 loại xuất hiện
    ...
    private IEnumerator ShowTetrisBlockHintDelayed(GameObject tetrisBlockObj)
    {
        ...
        StartCoroutine(ShowTetrisBlockHint(tetrisBlockObj));
    }
    ...
    {
        yield return new WaitForSeconds(delayBetweenHints);
        StartCoroutine(ShowBlockExpandGridHint(blockExpandGridObj));
    }
